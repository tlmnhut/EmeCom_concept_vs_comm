...
from archs import InformedSender, Receiver
...
def get_game(opt):
    ...
    receiver = Receiver(opt.game_size, feat_size,
                        opt.embedding_size, opt.vocab_size, reinforce=(opt.mode == 'rf')) # opts.mode
    if opt.mode == 'rf': # opts.mode
